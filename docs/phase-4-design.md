...
- **Stage 5 — Surfaces + management + polish.** Dashboard/YTD one-time-income lines (done in Stage 3); a "Skipped"/"Rules"/"Profiles" management area, empty/error states, mobile touch pass, accessibility. Plus the two additions below. *Checkpoint:* full loop on phone + desktop; desktop visuals unchanged where not import-related.
  ...
  **Stage 5 addition (PRIORITY) — smart column auto-guessing for unknown banks.** Goal: a first-time import from an *unrecognized* bank should almost always be a one-tap **"confirm ✓"**, not manual column mapping — so non-technical users rarely touch the mapping UI. When a file's header signature matches no saved/built-in profile, **don't drop to a blank mapping screen** — first *guess* the mapping with heuristics, then ask for confirmation.
  - **Heuristics (score each column from the header row + a sample of ~20 data rows):**
    - **Date column:** header name matches `/date|posted|trans(action)?/i`, and/or ≥~80% of sample values parse as a date (via the existing `extractStatementDate`, which already tolerates weekday prefixes like `Tuesday - 08/11/2026`). Highest score wins; also infer the date format (MM/DD/YYYY vs YYYY-MM-DD vs DD/MM/YYYY) from the sample.
    - **Amount column(s):** find columns whose values are numeric/currency (after stripping `+ - $ , ( )` via the existing `parseAmount`). Then decide the **mode**: **(a)** two such columns with opposing header hints (`/withdrawal|debit|payment|charge/i` vs `/deposit|credit/i`) *or* a near-mutually-exclusive fill pattern (each row populates exactly one) → **`debitCredit`** (map the debit/withdrawal col → money out, deposit/credit col → money in); **(b)** a single numeric column → **`signed`**, and infer `negativeMeans` from the sign distribution + header (a checking export is mostly spend, and negatives usually mean out).
    - **Description/merchant column:** the remaining non-date, non-amount text column with the longest average free-text / highest distinct-value cardinality.
  - **Confidence:** high when the date parses strongly, the amount mode is unambiguous, and a clear description column exists. **Low confidence → fall back to the full manual mapping UI** (never silently guess wrong).
  - **"Does this look right?" confirmation:** a friendly card with a **live signed preview** of the first ~5 rows — parsed date, cleaned merchant, and a signed amount with an obvious **money-in / money-out** chip — plus a big **"Yes, looks right"** button and a subtle **"Adjust"** link that opens the full manual mapping only if needed. Touch-friendly (≥44px targets); identical on desktop + web.
  - **On confirm:** save the guessed mapping as that bank's **import profile** (keyed by header signature, per household, synced → automatic and shareable next time — the same `ImportProfile` shape Stage 1 already defined). Manual mapping remains the fallback for low-confidence or "Adjust".
  - *Builds on existing pieces:* the `AmountMapping` three-mode model, `ImportProfile` + `headerSignature`/`detectProfile`, `parseAmount`, and `extractStatementDate` are already in place; this adds only the guesser + the confirm UI. *Checkpoint:* importing a brand-new bank's CSV (not PNC) lands on "Does this look right?" with correct signs, and one tap confirms + saves the profile.
