...
- **Stage 4 — Dedup polish + OFX/QFX.** Manual-entry match flag (§5b); OFX/QFX/QBO parser with FITID dedup. *Checkpoint:* a manually-entered duplicate is flagged; an OFX import is clean and de-dupes against a prior CSV of the same account.
- **Stage 5 — One-time-income surfaces + management + polish.** Dashboard/YTD lines, a "Skipped"/"Rules"/"Profiles" management area, empty/error states, mobile touch pass, accessibility. *Checkpoint:* full loop on phone + desktop; desktop visuals unchanged where not import-related.
